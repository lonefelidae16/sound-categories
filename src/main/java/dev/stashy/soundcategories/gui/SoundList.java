...
public class SoundList extends ElementListWidget<SoundList.SoundEntry> {
    ...
    public int addCenteredOption(GameOptions gameOptions, Option option) {
        return super.addEntry(SoundEntry.createCenteredOption(gameOptions, option, this.width));
    }
    ...
    protected static class SoundEntry extends ElementListWidget.Entry<SoundList.SoundEntry>
    {
        ...
        public static SoundEntry createOption(GameOptions options, Option option, int width)
        {
            var widget = option.createButton(options, width / 2 - 155, 0, 310);
            return new SoundEntry(List.of(widget));
        }

        public static SoundEntry createCenteredOption(GameOptions options, Option first, int width)
        {
            ClickableWidget widget = first.createButton(options, width / 2 - 75, 0, 150);
            return new SoundEntry(List.of(widget));
        }
